...
@property (strong, nonatomic) ServerManager*          vkManager;
...
@property (weak, nonatomic) IBOutlet UIView *viewForGesture;
...
- (void)viewDidLoad {
    ...
    [self.viewForGesture addGestureRecognizer:self.leftSwipe];
    
    self.rightSwipe.direction = UISwipeGestureRecognizerDirectionRight;
    [self.viewForGesture addGestureRecognizer:self.rightSwipe];
...
- (void)getLyricsForCurrentSong {
    ...
    if (self.currentSong.lyrics_id) {
        ...
        AccessToken* token = self.vkManager.token;
...
- (void)actionPlaySong:(UIButton*)sender {
    ...
    if ([self.songs count] - 1 > indexOfCurrentSong) {
...
- (void)actionShowToolsPopover:(UIBarButtonItem*)sender {
    ...
    UIAlertAction* deleteAllInfo = [UIAlertAction actionWithTitle:@"Delete (all info)"  style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
        ...
        Song* deletingSong = [self.songs objectAtIndex:index];
    ...
    UIAlertAction* addToPlaylist = [UIAlertAction actionWithTitle:@"Add to Playlist" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        PlaylistPopoverTableViewController* vc = [[PlaylistPopoverTableViewController alloc] initWithStyle:UITableViewStyleGrouped andBelongingSong:self.currentSong];
...
- (void)gestureRigthSwipe:(id)sender {
    
    [self actionPlayBackAndForward:self.playBackButton];
}

- (void)gestureLeftSwipe:(id)sender {
...
- (ServerManager*)vkManager {
    
    if (!_vkManager) {
        _vkManager = [ServerManager sharedManager];
    }
...
- (UISwipeGestureRecognizer*)leftSwipe {
    ...
    if (!_leftSwipe) {
        _leftSwipe = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(gestureLeftSwipe:)];
    }
    return _leftSwipe;
}
...
- (UISwipeGestureRecognizer*)rightSwipe {
    ...
    if (!_rightSwipe) {
        _rightSwipe = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(gestureRigthSwipe:)];
